 ...
 */
  ...
  class BF_Admin_Page_Class {
    ...
    public function plupload_head_js(){
      if ($this->has_field('plupload')){
         $plupload_init = array(
            ...
            'filters'             => array(array('title' => __('Allowed Files','pss'), 'extensions' => '*')),
            'multipart'           => true,
    ...
    public function edit_insert_to_post_text( $safe_text, $text ) {
      if( is_admin() && 'Insert into Post' == $safe_text){
        if (isset($_REQUEST['pss']) && 'insert_file' == $_REQUEST['pss'] )
          return str_replace(__('Insert into Post'), __('Use this File','pss'), $safe_text);
        else
          return str_replace(__('Insert into Post'), __('Use this Image','pss'), $safe_text);
      }
      return $safe_text;
    }
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" value="'.esc_attr(__('Save Changes','pss')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn"><br><br>
        </div>'."\n".'
      ...
      if ($this->saved_flag){
        ...
        if (is_array($this->errors) && count($this->errors) > 0 ){
          ...
          echo '<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.__('Settings saved.','pss').'</strong></div>'."\n";
        }
        echo '</div>'."\n";
      }
    ...
    echo '</div>'."\n".'<div style="clear:both"></div>'."\n".'<div class="footer_wrap">'."\n".'
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn" value="'.esc_attr(__('Save Changes','pss')).'" />'."\n".'
          <br><br>
        </div>'."\n".'
        <br style="clear:both"><br>
      </div>'."\n";
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image','pss' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete' ,'pss') . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.','pss')));
      die();
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove','pss').'" title="'.__('Remove','pss').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add','pss').'" title="'.__('Add','pss').'" id="add-'.$jsid.'"><br/></div>';
    ...
    echo '" alt="'.__('Remove','pss').'" title="'.__('Remove','pss').'" id="remove-'.$jsid.'"></div>'."\n";
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_plupload( $field, $meta) {  
    ...
    $html = '
        ...
        <div class="plupload-upload-uic hide-if-no-js '.$m1.'" id="'.$id.'plupload-upload-ui">
          <input id="'.$id.'plupload-browse-button" type="button" value="'.__('Select Files','pss').'" class="button" />
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( ! empty( $meta ) ) {
        $nonce = wp_create_nonce( 'at_ajax_delete' );
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files','pss') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete' ,'pss') . "</a>)</li>";
        }
        echo '</ol>';
      }
  ...
  public function show_field_media_manager($field,$meta){
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input class='at-delete_image_button button' type='button' rel='".$field['id']."' value='".__('Delete Image','pss')."' />"."\n";
    }else{
      ...
      $html .= "<input class='at-mm-upload_image_button button' type='button' rel='".$field['id']."' value='".__('Upload Image','pss')."' />"."\n";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='".$meta['id']."' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='".$meta['src']."' />";
      $html .= "<input class='at-delete_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".__('Delete Image','pss')."' />";
    }else{
      $html .= "<span class='mupload_img_holder'  data-wi='".$width."' data-he='".$height."'></span>";
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='' />";
      $html .= "<input class='at-upload_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".__('Upload Image','pss')."' />";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . __( 'Select a color' ,'pss') . "'/>"."\n";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'pss') . "'/>";
  ...
  public function show_import_export(){
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.__('Export','pss').'</h3>
        <p>'. __('To export saved settings click the Export button bellow and you will get the export Code in the box bellow, which you can later use to import.','pss').'</p>
        <div class="export_code">
          <label for="export_code">'. __('Export Code','pss').'</label><br/>
          <textarea id="export_code"></textarea>        
          <input class="button" type="button" value="'. __('Get Export','pss').'" id="apc_export_b" />'.$this->create_export_download_link().'
          <div class="export_status" style="display: none;"><img src="'.$this->SelfPath.'"/images/loading.gif" alt="loading..."/></div>
          <div class="export_results alert" style="display: none;"></div>
        </div>
      </div>
      <div style="padding 10px;" class="apc_import"><h3>'.__('Import','pss').'</h3>
        <p>'. __('To Import saved settings paste the Export output in to the Import Code box bellow and click Import.','pss').'</p>
        <div class="import_code">
          <label for="import_code">'. __('Import Code','pss').'</label><br/>
          <textarea id="import_code"></textarea>
                  <input class="button" type="button"  value="'. __('Import','pss').'" id="apc_import_b" />
          <div class="import_status" style="display: none;"><img src="'.$this->SelfPath.'"/images/loading.gif" alt="loading..."/></div>
  ...
  public function export(){
    ...
    if (!isset($_GET['group'])){
      $re['err'] = __('error in ajax request! (1)','pss');
    ...
    if ($options !== false)
      ...
      $re['err'] = __('error in ajax request! (2)','pss');
  ...
  public function import(){
    ...
    if (!isset($_POST['imp'])){
      $re['err'] = __('error in ajax request! (3)','pss');
    ...
    if (is_array($import_code)){
      ...
      $re['success']= __('Setting imported, make sure you ','pss'). '<input class="button-primary" type="button"  value="'. __('Refresh this page','pss').'" id="apc_refresh_page_b" />';
    }else{
      $re['err'] = __('Could not import settings! (4)','pss');
    }
    //update_nonce
      $re['nonce'] = wp_create_nonce("apc_import");
    echo json_encode($re);
  ...
  public function download_file($content = null, $file_name = null){
    ...
    if ( !current_user_can('edit_themes') )
        wp_die('<p>'.__('You do not have sufficient permissions to edit templates for this site.','pss').'</p>');
    
    if ($content === null || $file_name === null){
        wp_die('<p>'.__('Error Downloading file.','pss').'</p>');     
    }
  ...
  public function create_export_download_link($echo = false){
    ...
    if ($echo === true)
        echo '<a href="'.$export_url.'" target="_blank">'.__('Download Export','pss').'</a>';
    elseif ($echo == 'url')
        return $export_url;
    return '<a class="button" href="'.$export_url.'" target="_blank">'.__('Download Export','pss').'</a>';
  }
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'pss', dirname(__FILE__) . '/lang/' . get_locale() .'.mo' );
  }
    ...
    foreach ($field['validate'] as $type => $args) {
      if (method_exists($this,'is_' . $type)){  
        if (call_user_func ( array( $this, 'is_' . $type ), $meta ,$args['param']) === false){
          ...
          $this->errors[$field['id']]['m'][] = (isset($args['message'])? $args['message'] : __('Not Valid ','pss') . $type);
          $ret = false;
        }
      }
    }
  ...
  public function displayErrors(){
    if ($this->errors_flag){
      ...
      echo '<h4>'.__('Errors in saving changes', 'pss').'</h4>';
